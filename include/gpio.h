/*
 * gpio.h — GPIO driver for CH32V003
 *
 * Provides pin initialisation, atomic set/reset output, and single-pin
 * input read.  All raw register access lives in gpio.c; callers use
 * only the constants and function prototypes defined here.
 *
 * Supported ports: A (PA1-PA2), C (PC0-PC7), D (PD0-PD7)
 */
...
/* ── Pin mode constants (CNF:MODE encoding for GPIOx_CFGLR) ───────── */
/*
 * Each pin occupies 4 bits in GPIOx_CFGLR:
 *   [3:2] CNF   [1:0] MODE
 *
 * Input  (MODE=00): CNF=01 → floating, CNF=10 → pull-up/pull-down
 * Output (MODE=11, 30 MHz slew): CNF=00 → push-pull
 */
